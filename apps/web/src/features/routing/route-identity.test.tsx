...
describe('the headline', () => {
  ...
  it('says "same length" only under display precision, and says so', () => {
    ...
    render(
      <RouteComparisonView
        comparison={comparison(
          route({ distance_m: 300 }),
          route({ distance_m: 300.3, coordinates: DETOUR }),
          0.3,
        )}
      />,
    );
  ...
  it('does not infer equality from a missing difference', () => {
    render(
      <RouteComparisonView
        comparison={comparison(route(), route({ coordinates: DETOUR }), null)}
      />,
    );
    const status = screen.getByRole('status');
  ...
  it('says "the same route" from identity, and only when the numbers agree', () => {
    const shared = [segment('e1', DIRECT)];
    const { unmount } = render(
      <RouteComparisonView
        comparison={comparison(route({ segments: shared }), route({ segments: shared }), 0)}
    ...
    expect(screen.getByRole('status')).toHaveTextContent(/same route/i);
    unmount();

    // Identical segments beside a reported 40 m difference is a response that
    // contradicts itself. The page reports the figure it was given and claims
    // no identity.
    render(
      <RouteComparisonView
        comparison={comparison(route({ segments: shared }), route({ segments: shared }), 40)}
      />,
    );
    expect(screen.getByRole('status')).toHaveTextContent(/40 m longer/);
    expect(screen.getByRole('status')).not.toHaveTextContent(/same route/i);
  });
});
...
describe('what the uncertainty line may claim', () => {
  ...
  it('reports "no gaps" as a statement about the record, not as everything recorded', () => {
    ...
    render(<RouteComparisonView comparison={comparison(route(), complete, 5)} />);
    ...
    expect(screen.getByTestId('steepest-climb')).toHaveTextContent('Estimated from elevation');
    expect(screen.getByTestId('gradient-provenance')).toHaveTextContent(
      /elevation model of the ground/i,
    );
  });
  ...
  it('keeps the gaps unknown when the response carries no coverage at all', () => {
    ...
    render(<RouteComparisonView comparison={comparison(route(), uncovered, 5)} />);
  ...
  it('names the crossings denominator when kerb is the largest gap', () => {
    ...
    render(<RouteComparisonView comparison={comparison(route(), kerbs, 5)} />);
